...
------------------------------------------------------------------------------------------------------------------------------------------------
--PROD
-- Residential Tenancy - 012Nz000005ywr3IAA  -- Set across the tabes
-- Residential Contract - 012Nz0000050AZCIA2 -- Set on Sub_Contract creation 
------------------------------------------------------------------------------------------------------------------------------------------------
...
FROM (
    ...
    FROM (
        SELECT 
            ...
            '012Nz000005ywr3IAA' AS RecordTypeId,
...
JOIN sub_Tenancies ctr 
    ...
    AND ctr.RecordTypeId = '012Nz000005ywr3IAA' -- Only link to "Residential Tenancy" contracts
...
WHERE sub_tenancies."PMS_Reference__c" = latest."PMS_Reference__c"
  AND sub_tenancies.RecordTypeId = '012Nz000005ywr3IAA';
...
FROM (
    ...
    JOIN sub_Tenancies primary_contract ON primary_contract.AccountId = licence.AccountId
        WHERE COALESCE(primary_contract."Agreement_Type__c", '012Nz000005ywr3IAA') NOT IN (
